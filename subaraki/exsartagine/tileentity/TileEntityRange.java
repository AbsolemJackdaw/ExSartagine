			...
			if(fuelTimer == 0)
			{
				for(int i = 0 ; i < inventory.getSlots() ; i++)
				{
					...
					if(!stack.isEmpty() && TileEntityFurnace.isItemFuel(stack))
					{
						...
						world.notifyBlockUpdate(getPos(), world.getBlockState(getPos()), ExSartagineBlock.range.getDefaultState(), 3);
						break;
					}
				}
			}
		...
		if(!connected.isEmpty())
		{
			for(BlockPos posTere : connected)
			{
				...
				if(te instanceof TileEntityRangeExtension){
					...
					world.notifyBlockUpdate(posTere, world.getBlockState(posTere), ExSartagineBlock.range_extension.getDefaultState(), 3);
